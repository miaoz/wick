...
- **交易所仓位（可选）**：每个日记本可绑定一个账号（Binance USDⓈ-M / OKX 永续 / Hyperliquid）。只读凭据仅存本机（打包应用走钥匙串，`swift run` 走本地文件以免每次重编译弹密码），仓位按「开仓日期 + 标签」挂到条目上；开仓日没写日记会自动补建条目（详见 [交易所仓位同步](#交易所仓位同步)）
...
- **同步范围**：与 Binance 相同——从该日记本**最早一篇日记**起（没有日记时近 180 天），更早的成交不拉。日常增量刷新（30 分钟 + 打开日记）。OKX / Hyperliquid 所侧另外还有深度上限（大约 3 个月 / 最近 1 万笔）。
- **展示**：按「开仓日期 + 标签」挂到条目上。标签 `BTC` 可匹配 `BTCUSDT` / `BTCUSDC` / Hyperliquid 的 `BTC`。
- **自动补条目**：开仓日没有日记时自动创建（每币种一条），标签沿用你惯用写法否则用基础币种；**删除的自动条目不会复活**。
- **只读不写**：不改已有日记正文和标签。
