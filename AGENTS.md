...
| `WickTheme.swift` | **仅剩 `WickPrintFont`（40 行）**：纸面字体解析（委托 `AppFont.paperNSFont`；缓存键必须包含所选 PostScript 名，字体暂不可用时禁止缓存系统回退）。**主题引擎已下沉到 WickCalendarKit 同名文件**（见下），双 target 同名文件注意分辨 |
...
| `PaperChrome.swift` / `TagChipFlow.swift` / `TableViewSelectionSuppressor.swift` / `JournalPnlCalendarView.swift` / `JournalDatePickerView.swift` | 纸质外壳件(撕边/胶带/刻印小方钮 `InkIconButton`——常态无框融入纸面,hover 才浮 1pt 烛火线+起光/烛印方砖);标签签条(纸面朱砂方角);NSTableView 高亮抑制;盈亏月历(红盈 `pnlUp`/黛亏 `pnlDown`/有日记无仓位烛痕渍 `stain1`,软填充 ≈14%,**周一开头**、星期头随 App语言,‹ › 翻月、「已实现合计」行、今日烛火描边、点选跳当天);**秉烛风格日期选择器**(`JournalDatePickerView`,无系统蓝框/纸面年月/周一开头/烛火选中与今日描边/返回今日) |
...
| `LaunchAtLogin.swift` / `UpdateChecker.swift` / `AppInfo.swift` / `AppNotifications.swift` / `Exports.swift` / `JournalReminderScheduler.swift` / `WickDateFormat.swift` | 小工具集：登录项 / 检查更新 / 版本比较 / 共享通知名 / `@_exported` / 每日通知（含包形态门控）/ 日期格式化辅助 |
...
| `TradingCalendarRootView.swift` | 撕页根视图，平台无关（`init(language:onClose:onPageTorn:layout:)`）：裂纹模型、纹理快照随日期/数据/版号/栏目重刷、轻点栏目行切 tab、轻点行区翻页（位移 <8pt）、撕页经 `onPageTorn` 交宿主（碎页携带当日两栏数据 + 版号 + 栏目）；`.contextMenu` 分享本页/拷贝图片（macOS 右键整页可出；iOS 长按只有页面上半部有效——撕页区 `DragGesture(minimumDistance: 0)` 会吃掉长按），渲染走 `CalendarSnapshot` 同一管线 |
| `ImageShare.swift` | 跨平台图片投递：剪贴板拷贝 + 系统分享面板（macOS `NSSharingServicePicker` / iOS `UIActivityViewController`）；黄历页与双端仓位单据（`JournalExchangePositions` / `PhoneReceiptCard`）共用，单据渲染用 `ImageRenderer` 且须显式注入 environment（渲染树不继承外部环境）；**投递必须带渲染 scale**（`UIImage(cgImage:)` 默认 scale=1、`NSImage` size 须为像素/scale），否则接收方按 1x 栅格化发虚 |
...
| `TraderAlmanac.swift` | 交易员黄历宜忌引擎（`TraderAlmanac.entry(for:events:)`，936 行系统：宜忌/吉神/煞位/方印 + 五类签池 + 周末/宏观大日/周五/周一分流；**种子为纯整数 `year*10000+month*100+day`**，跨进程确定性——勿改回 `Hasher`）；配套视图 `TraderYiJiRow`/`TraderAlmanacMetaRow`/`TraderAlmanacSealBadge` |
| `WickTheme.swift` | **「秉烛」主题引擎本体**（WickCore 同名文件只剩 40 行 `WickPrintFont`，双 target 同名文件注意分辨）：`WickRGB`、`WickPalette`（宣纸/烟墨/烛火/朱砂/黛青/烛痕/单据纸/烛印方砖）、`DayPhase` 四锚点 × 亮暗插值、火苗色跨时段恒定、`WICK_ARC_TIME=HH:mm`（仅 DEBUG）、`\.wickPalette` 环境键、纸面层级映射 `columnPaper`/`editorCanvas`/`pageSurface`；设计蓝本 `designs/wick-design-language/final.html` |
| `JournalReviewBadge.swift` / `BurnStripView.swift` | 白文方章（`✓/✗` 涨跌色跟随 `pnlColorConvention`、残边蘸印 `SealBodyShape`）；烛痕条（暖渍+烛苗、刻度参数化、形变只作用剪影层）。两者均已迁出 WickCore |
...
其他目录：`assets/`（图标，iconset 为中间产物）、`ios/`（iPhone 客户端 v0，仅中文 UI：手写 xcodeproj 用文件系统同步组、`Info.plist` 在 `ios/` 根而非同步组内；链接 `WickSync`+`WickCalendarKit`+`WickTrading`；`CalendarView` 满屏承载 kit 根视图并构造 `PaperLayout.fullScreen`；DEBUG 启动参数 `-wick-open-calendar` 直接弹日历便于截图；CLI 校验走 **scheme**：`xcodebuild -project ios/WickPhone.xcodeproj -scheme WickPhone -destination 'generic/platform=iOS' build CODE_SIGNING_ALLOWED=NO`（**勿用** `-target WickPhone`，App target 无法解析 `WickCalendarKit`/`WickSync`）；iOS Store 回归测试 target 为 `WickPhoneTests`，模拟器命令：`xcodebuild test -project ios/WickPhone.xcodeproj -scheme WickPhone -destination 'platform=iOS Simulator,id=<device-id>' CODE_SIGNING_ALLOWED=NO`、`scripts/`（`package_app.sh`/`package_zip.sh`/图标生成）、`test/`（`agent-readiness.test.mjs`：落地页中间件回归，`node --test` 运行）、`functions/`（Cloudflare Pages Functions：`api/wishlist.js` 心愿单 + `_middleware.js` 内容协商/安全头，`landing/**`+`functions/**` 变更走 `landing.yml` 部署前先跑测试）、`landing/`（官网落地页，静态站：`index.html`+`app.js`+`style.css`+`assets/`，中文只存 HTML 一份、`app.js` 的 `I18N_EN` 只放英文、启动时从 DOM 捕获中文；截图素材为模拟器 3x / 虚拟 2x 屏直出；**宋体印刷字 = 自托管子集 `assets/fonts/wick-print.woff2`**（Noto Serif SC 可变字重 200–900、改名 Wick Print、OFL 见同目录 `OFL.txt`，`--f-print` 栈置顶，`index.html` preload）——**iOS 无预装中文宋体**（Songti SC 是按需下载资产），不自带字体必混字（日文明朝缺简体特有字，图/载/烛等会掉回退）；**改落地页文案后必须跑 `scripts/subset_landing_font.sh` 重新子集化**（字符集 = index.html+app.js 全集，缓存在 `.build/landing-font/`；字体源已 pin 到 google/fonts 具体 commit，勿改回 `main`）；`_d_meta.json` 是设计工具元数据，`.assetsignore` 拦截不上传；Cloudflare Pages 项目 `wick` → <https://wick-ccc.pages.dev>，根 `wrangler.toml` 的 `pages_build_output_dir` 指向它，本地部署 `npx wrangler pages deploy landing --project-name=wick`，凭据在根 `.env`[已 gitignore]）、`.github/workflows/`（`release.yml` 应用发布 + iOS 模拟器回归、`landing.yml` 落地页部署：`landing/**` 变更推 main 即自动部署，用仓库 secrets `CLOUDFLARE_API_TOKEN`/`CLOUDFLARE_ACCOUNT_ID`）、`dist/`（产物，已 gitignore）。
...
- CI（release.yml）：macos-26 + Xcode 26.6 → `swift test` → iOS Simulator `WickPhoneTests`（CI-01）→ 打包 → 仅 tag `v*` 建 GitHub Release（正文由 git log 自动生成：自上一 tag 的改动清单 + 安装说明，**只留最新 10 个**，不删 git tag）+ **自动上传包到 Cloudflare R2 桶 `application-releases/wick/`**（`Wick-macOS-${VERSION}.zip` 与最新别名 `Wick.zip`，公网直链 `https://dl.bitfroth.com/wick/Wick.zip`，由 `landing/` 下载按钮直接引用；本地手动上传可用 `./scripts/upload_r2.sh`）。landing.yml 部署前先 `node --test`（CI-01）。
- 测试落点：纯计算进可注入 `Date`/`Calendar` 的静态方法（`TimeProgressCalculator`/`DayArcEngine`/`PaperSim` 等）；存储行为进 `JournalStoreTests`；同步分支一律用 `WickSyncTests` 的假后端复现（假后端忠实模拟 Dropbox：分块哈希、增量回声——**不碰网络**）；iOS Store 回归在 `WickPhoneTests`（模拟器 `xcodebuild test`）；落地页/中间件在 `test/agent-readiness.test.mjs`（`node --test`）；另有少量视图级测试（`TraderAlmanacTests` 等），但 UI 交互路径仍以人工验证为主。
