...
| `LegacyJournalToolbar.swift` | macOS 13 工具栏代理（`LegacyJournalToolbarDelegate`：折叠钮最左、日记本控件其次、新建钮最右；折叠走响应链 `toggleSidebar:`；日记本控件为**图标+当前日记本名的自定义 `NSButton`**，点击弹出共享 `NSMenu`（`NSMenuToolbarItem` 无法显示文字），名称随 `wickActiveJournalDidChange` 刷新；菜单动作经 Notification 异步交给 SwiftUI 弹窗——同步派发会被 macOS 13 的菜单跟踪吞掉） |
