...
| `WickTrading` | 纯 Foundation + CryptoKit：Binance 客户端（签名/分块分页/错误映射，transport 可注入）+ 成交→仓位聚合 + 宽松标签匹配 + `DailyRealizedPnl`（按本地天分桶的已实现盈亏求和，供侧栏月历着色） |
...
| `JournalRootView.swift` / `JournalSidebarView.swift` / `JournalEditorPane.swift` | 秉烛主窗:**全宽顶栏**(红绿灯让位、三态循环钮 ⌃⌘S[全导航→仅列表→专注]、**静态日记名**(顶栏不挂日记本操作;切换 = 栏一点行,新建 = 栏一「日记本」节头右侧 + 钮,重命名/删除 = 日记本行右键菜单,删除仅限非最后一本)+ 选中日小注、搜索、新建、右钮 ⌥⌘0;**顶栏内容行垂直位置**靠 `TrafficLightProbe` 实测红绿灯中心(close 钮转 window 基坐标取中心,转 contentView 会拿到 hosting 视图的过期帧,勿用),整行再往下偏 ~3.5pt(原生统一工具栏里内容也坐在灯线偏下,完全同线反而读起来偏上),上下留白相等)+ **手动三栏**(导航 `JournalNavigationSidebar` / 日期列表 `JournalDayListColumn` / 编辑页;**不用 NavigationSplitView**——macOS 26 会把侧栏画成浮动圆角卡片,与平铺纸面冲突;栏间 `JournalColumnDivider` = 1pt 界线 + 7pt 命中区,拖拽调宽、拖到边缘/双击收栏,栏宽落 `wick.journal.navWidth/listWidth`;**栏一文字左缘固定 x=10,与首颗红绿灯左缘(x≈9)对齐**(v4 同此关系,行 pill 留 4pt 外边距);标签签条走 `SidebarChipFlow`(自研 Layout 贪心换行+超长截断,**勿用 LazyVGrid adaptive**——等宽列会让长签溢出、拖栏时与邻签重叠))+ 栏四检查器 `JournalInspectorView`(今日事件 + 盈亏月历,仅彩蛋关时在;**盈亏收合 → 事件栏全量滚动撑满整栏**,事件常有数十项;宏观标题最多两行、财报代码 `fixedSize` 单行不折);栏位状态持久化(`journalColumnMode`);**编辑页 = 一天一页纸**(`pageSurface` 纸页 + 衬线大日期页眉[点开改日]+ 星期/农历小注 + 当日已实现盈亏 + 页内细烛痕条——过去的天然燃尽、今日带「已过 N%」小字),条目沿发丝线排列、无卡片壳,编辑栏有 440pt 宽度地板(`editorMinWidth`,再窄页眉就溢出变形);**日期列表按月分节**(「八月 2026」节头),行 = 日期+周几 / 条数·平仓数 / 等宽盈亏+复盘小章,选中 = 页纸高亮 + 左缘 3pt 朱砂界,List 必须 `.scrollContentBackground(.hidden)` 否则侧栏 vibrancy 把纸色变灰;DEBUG `-wick-journal-detail-only` 专注态启动 |
...
| `JournalWindowController.swift` | 手动持有日记 `NSWindow` + 激活策略切换；**全版本无 toolbar**(KVO 钉住 `window.toolbar` = nil)——窗口控件全部在全宽顶栏(`JournalRootView.topBar`),栏位折叠走三态循环(`journalColumnMode`);`hosting.safeAreaRegions = []`(13.3+)让顶栏与红绿灯同一行,否则 SwiftUI 安全区会把顶栏挤到红绿灯下面;**窗口最小宽 = 可见栏位最小值之和 + 编辑栏地板**(`requiredMinWidth`,随栏位/检查器开关重算),且 `windowDidResize` 兜底强拉——程序化 setFrame(autosave 恢复、content-size 跟踪)**不吃 minSize**;DEBUG 裸二进制的 UserDefaults 落在 `Wick` domain(与打包的 com.miaoz.wick 互不相通) |
