...
- Bundle ID：`com.miaoz.wick`；当前版本默认 `1.9.4 (35)`（见 `scripts/package_app.sh` 中的 `VERSION`/`BUILD` 默认值）。
...
| `JournalSyncState.swift` | 远端布局 `/journals/<uuid>/{manifest.json,days/,images/,tombstones/,conflicts/,settlements/}`；manifest/墓碑/冲突/结算标记载荷 Codable（败方归档仅在用户处理冲突前保留，处理后由清理队列删除）；每设备同步状态（cursor、远端文件视图（rev + 元数据哈希，元数据哈希仅作调试、**永不与本地哈希比较**）、按天 `DaySyncState`（哈希一律本地规范约定、**远端变更只比 rev**；`pushedHashes` 记本设备最近 5 个推送哈希用于自合并保护；`settlement: DaySettlement`（`pushSettled`/`adoptRemote`/`markSettled`）为待执行的用户结算决策，旧字段 `settledPushHash`/`settleAdoptRemote`/`settleMarkHash` 解码时迁移、不再编码）、pendingConflicts、冲突归档清理队列、日记名基线 `manifestName`；**自定义 `init(from:)` 全字段带默认值，旧状态文件可解码**）与本地持久化（`~/Library/Application Support/Wick/SyncState/<uuid>.json`，**不参与同步**） |
| `JournalSyncEngine.swift` | 对账引擎（`@MainActor ObservableObject`），单写者无感性建立在三条不变量上：**①rev 回声抑制**--远端变更检测只比 rev（自己的上传会记录服务端返回的 rev，增量回声带回同 rev 即视为未变；本地规范哈希永不与 Dropbox `content_hash` 比较，两算法永不相等）；**②拉取即固定点**--pull 后基线用下载字节本地重算的规范哈希，重哈希必相等，收端绝不会把刚拉到的内容再推回去；**③绝不和自己冲突**--`mergeDay` 发现「远端」其实是本设备曾推送过的版本（`pushedHashes` 命中：过期回声撞上新编辑、或同伴原样转推）时直接重推本地，不归档不弹冲突；冲突只留给真正的跨设备分歧。远端内容应用前还有**新鲜度守卫**（应用/合并基于周期开头的快照，快照后用户又改过的天本轮跳过、下轮以新本地重新决策，杜绝中途打字被覆写）。流程：cursor 增量 -> manifest `formatVersion` 版本门 + 日记名对账（本地改名->rev 条件写推送；远端 manifest 被改写->`applySyncedJournalName` 本地采用；双改->后推者胜、败方下轮采用；旧状态文件无 `manifestName` 基线则一次性播种--远端未动时本地未推送的改名补推、远端已动则信任远端）-> 发现其他日记本 manifest（`discoveredJournals`，供自动/手动导入，消失即剪除）-> 每天先执行待定 `DaySettlement`（用户决策以**权威动作**短路对账矩阵、绝不重新合并：`.local` 权威推送所选版本--409 则取新 rev 覆写，用户选择高于合并收敛；`.remote` 采用 Dropbox 当前内容；`.merged` 仅上传结算标记后照常跑矩阵收敛本机；若用户决策后又编辑了同一天则决策被新内容取代，回退矩阵正常同步、事后补标记）-> 按天矩阵（本地变->条件上传；远端变->下载应用；双变->条目并集合并，败者存档 `conflicts/` 并出 `pendingConflicts`（记录同时携带 本机/远端/合并 三版 `JournalEntry`，设置页经 `SyncConflictResolutionView` 展开对比并一键选择 `resolveConflict(.local/.remote/.merged)`；两台机器先后各自解决同一批冲突时不再互相触发重合并））；**冲突一经处理（选择或忽略）即把败方归档排入 `pendingConflictCleanups`，下一轮周期删除远端 `conflicts/` 归档**--归档路径带 UUID 后缀防同秒碰撞，归档上传失败（尽力而为）不阻断合并；**解决端会上传一个结算标记**（`settlements/<dayKey>-…json`：dayKey + 结算后的**本地规范**内容哈希 + 设备 + 时间戳），其它设备在本轮矩阵之后用自己 `remoteContentHash`（同样本地规范约定）比对，命中即自动清掉过时记录--单人在一台设备解决后，其它在线设备无需手动清空、自动收敛；标记按 `tombstoneRetention` GC；本地删->先写墓碑再删远端；远端墓碑->本地删（本地有改动则改动方胜并清墓碑）；远端文件无墓碑消失->视为事故自动回传，绝不镜像删除 -> 图片按引用差集上传/下载 -> 墓碑 30 天 GC；60s 周期 + 15s 防抖 + `syncOnce()`（退出用）+ `resetSyncState()`（重导入前调） |
